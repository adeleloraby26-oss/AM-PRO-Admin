...
async function saveUser() {
  ...
  const { error: lvlErr } = await sb.from("users").update({ level: modalLevel }).eq("id", activeUserId);
  if (lvlErr) { toast("Error updating level: " + lvlErr.message, "error"); return; }

  // Update password via admin API if provided
  if (newPass) {
    const res = await fetch("https://tzojjwnqodcrhwjaasja.supabase.co/auth/v1/admin/users/" + activeUserId, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
        "apikey": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InR6b2pqd25xb2Rjcmh3amFhc2phIiwicm9sZSI6ImFub24iLCJpYXQiOjE3Nzc2NzA2ODAsImV4cCI6MjA5MzI0NjY4MH0.G4IGSUgjVIKTNVszU5GpxNaD0VUnSmzUXe8p7uUl418",
        "Authorization": "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InR6b2pqd25xb2Rjcmh3amFhc2phIiwicm9sZSI6ImFub24iLCJpYXQiOjE3Nzc2NzA2ODAsImV4cCI6MjA5MzI0NjY4MH0.G4IGSUgjVIKTNVszU5GpxNaD0VUnSmzUXe8p7uUl418"
      },
      body: JSON.stringify({ password: newPass })
    });
    if (!res.ok) {
      // Fallback: note that direct password reset via anon key is limited
      // Admin must use service_role key for this endpoint
      toast("Level saved! Password update requires service_role key in Supabase.", "error");
      closeUserModal();
      await loadUsers();
      return;
    }
...
function renderTasksList(tasks) {
  ...
  list.innerHTML = tasks.map(t => `
    <div class="task-row">
      ...
      <div class="task-info">
        <div class="task-title">${t.text || "Untitled Task"}</div>
        <div class="task-assignee">Assigned to: ${t.users ? (t.users.name||t.users.username) : "All users"}</div>
      </div>
...
async function addTask() {
  ...
  if (!text) { toast("Please enter task text.", "error"); return; }

  const rows = userId
    ? [{ text, user_id: userId }]
    : allUsers.map(u => ({ text, user_id: u.id }));
